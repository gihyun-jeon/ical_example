...
public class IcalParse {
    ...
    private static final long ONE_DAY_MSEC = 60 * 60 * 24 * 1000;
    ...
    private List<SimpleScheduleModel> parseRepeatVEvent(List<VEvent> 반복_모일정과_반복예외_일정리스트, ZonedDateTime from, ZonedDateTime until, ZoneId zoneId) {
        ...
        for (VEvent vEvent : 반복_모일정과_반복예외_일정리스트) {
            ...
            // "from" 부터 "until" 까지만 반복을 풀면 되니까,
            // 불필요하게 처음부터 반복 풀지 말고, 적당히 jump 한다.
            it.advanceTo(new Date(from.toEpochSecond() * 1000 - ONE_DAY_MSEC)); // 하루 정도 버퍼를 두고, DateIterator 를 앞으로 당긴다.
